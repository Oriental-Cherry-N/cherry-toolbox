...
If WScript.Arguments.Count > 1 Then
  MsgBox "Usage: start-source.vbs [--hidden | --check]", vbExclamation, "Cherry Toolbox"
  WScript.Quit 2
End If
...
If WScript.Arguments.Count = 1 Then
  ...
  If argument <> "--hidden" And argument <> "--check" Then
    MsgBox "Unsupported argument: " & WScript.Arguments(0), vbExclamation, "Cherry Toolbox"
    WScript.Quit 2
  End If
End If
...
If Not fileSystem.FileExists(scriptPath) Then
  MsgBox "start-source.cmd was not found.", vbCritical, "Cherry Toolbox"
  WScript.Quit 1
End If

processEnvironment("CT_NO_PAUSE") = "1"
quote = Chr(34)
...
If exitCode <> 0 Then
  MsgBox "Source startup failed with exit code " & exitCode & "." & vbCrLf & _
    "Run start-source.cmd --check for details.", vbCritical, "Cherry Toolbox"
End If
